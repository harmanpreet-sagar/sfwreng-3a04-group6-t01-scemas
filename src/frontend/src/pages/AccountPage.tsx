 ...
 *
 * Tabs (admin only sees all three, operators only see Accounts):
 *  1. Accounts        — account table + create form (admin) or own account (operator)
 *  2. Pending Requests — approve/deny registration requests (admin only)
 *  3. Audit Log       — filterable event log (admin only)
 */
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-[4.25rem] flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <span className="hidden md:block text-sm text-moss-100/95 truncate max-w-[200px]">
              {self?.name}
              <span className={`ml-2 text-[10px] uppercase tracking-wide px-2 py-0.5 rounded-md font-bold ${
                isAdmin
                      ...
                      {accounts.map(acc => (
                        <tr key={acc.aid} className="hover:bg-parchment/40 transition-colors">
                          ...
                          <td className="px-5 py-3.5">
                            <span className={`inline-flex items-center rounded-lg px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide ${
                              acc.clearance === 'admin'
                    ...
                    <tbody className="divide-y divide-ink-100">
                      {auditLog.length === 0 ? (
                        ...
                        <tr key={entry.id} className="hover:bg-parchment/40 transition-colors">
                          <td className="px-5 py-3.5">
                            <span className={`inline-flex items-center rounded-lg px-2.5 py-1 text-[11px] font-bold font-mono uppercase tracking-wide ${
                              entry.event_type.includes('failure') || entry.event_type.includes('denied')
